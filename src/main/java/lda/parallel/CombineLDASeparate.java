 ...
 */
public class CombineLDASeparate {
  ...
  public CombineLDASeparate(int D, int V, int K, float alpha, float beta,
  ...
  public void buildMat(List<Document> docs) {
    ...
    System.out.format("ftreeNum=%d ftreeTokens=%d WarpNum=%d WarpTokens=%d MH=%d threshold=%d\n",
            shortDocs.size(), sn, longDocs.size(), ln, MH_STEPS, threshold);
  }
...
//    Random random = new Random(System.currentTimeMillis());
  ...
  public double trainLong(int it) {
    return lda.jni.Utils.warpOneIterSep(it, D, V, K, N,
            ld, ln, sd, sn,
            lws, lds, lwidx, nk,
            sws, stopics, ltopics, isShort,
            mhs, MH_STEPS, alpha, beta, vbeta, maxlen);
  ...
  public static CombineLDASeparate read(String[] argv) throws IOException {
    String path = argv[0];
    ...
    CombineLDASeparate lda = new CombineLDASeparate(M, V, K, alpha, beta, docs, taskNum, length, mh_steps);
    return lda;
  }

  public static void cmds(String[] argv) throws IOException {
    CombineLDASeparate lda = read(argv);
